...
#            ddy[-1,:]=(rhov[-1,:]-rhov[-2,:])/(dy[-1,:])
    ...
    def Calculate_Stress(self, u, v, dx, dy, stress):
        mu=self.Domain.mu
        ...
        if stress=='11':
            ...
            tau[:-1,-1]+=mu*0.5*((u[:-1,-1]-u[:-1,-2])/(dx[:-1,-1])+\
                    (u[1:,-1]-u[1:,-2])/(dx[1:,-1]))
            tau[1:,-1]-=mu*0.5*((u[1:,-1]-u[1:,-2])/(dx[1:,-1])+\
                    (u[:-1,-1]-u[:-1,-2])/(dx[:-1,-1]))
        # Off-diagonal stresses
            ...
            tau[:-1,-1]+=mu*0.5*((v[:-1,-1]-v[:-1,-2])/(dx[:-1,-1])+\
                    (v[1:,-1]-v[1:,-2])/(dx[1:,-1]))
            tau[1:,-1]-=mu*0.5*((v[1:,-1]-v[1:,-2])/(dx[1:,-1])+\
                    (v[:-1,-1]-v[:-1,-2])/(dx[:-1,-1]))
            
            tau[:-1,0]+=mu*0.5*((v[:-1,1]-v[:-1,0])/(dx[:-1,0])+\
                    (v[1:,1]-v[1:,0])/(dx[1:,0]))
            tau[1:,0]-=mu*0.5*((v[1:,1]-v[1:,0])/(dx[1:,0])+\
    ...
    def Source_CSWork(self, u, v, dx, dy, Ax, Ay):
        mu=self.Domain.mu
        work=np.zeros_like(dx)
        # tau 11 * u
        work[:,:-1]+=(4.0/3*mu*(u[:,1:]-u[:,:-1])/(dx[:,:-1]))\
                *0.5*(u[:,:-1]+u[:,1:])*Ax[:,:-1]
        work[:,1:] -=(4.0/3*mu*(u[:,1:]-u[:,:-1])/(dx[:,:-1]))\
                *0.5*(u[:,1:]+u[:,:-1])*Ax[:,1:]
        work[1:-1,:-1]+=(2.0/3*mu*0.5*((v[2:,:-1]-v[:-2,:-1])/(dy[1:-1,:-1]+dy[:-2,:-1])+\
                (v[2:,1:]-v[:-2,1:])/(dy[1:-1,1:]+dy[:-2,1:])))\
                *0.5*(u[1:-1,:-1]+u[1:-1,1:])*Ax[1:-1,:-1]
        work[1:-1,1:]-=(2.0/3*mu*0.5*((v[2:,1:]-v[:-2,1:])/(dy[1:-1,1:]+dy[:-2,1:])+\
                (v[2:,:-1]-v[:-2,:-1])/(dy[1:-1,:-1]+dy[:-2,:-1])))\
                *0.5*(u[1:-1,:-1]+u[1:-1,1:])*Ax[1:-1,1:]
        work[0,:-1]+=(2.0/3*mu*0.5*((v[1,:-1]-v[0,:-1])/(dy[0,:-1])+\
                (v[1,1:]-v[0,1:])/(dy[0,1:])))\
                *0.5*(u[0,:-1]+u[0,1:])*Ax[0,:-1]
        work[0,1:]-=(2.0/3*mu*0.5*((v[1,:-1]-v[0,:-1])/(dy[0,:-1])+\
                (v[1,1:]-v[0,1:])/(dy[0,1:])))\
                *0.5*(u[0,:-1]+u[0,1:])*Ax[0,1:]
        work[-1,:-1]+=(2.0/3*mu*0.5*((v[-1,:-1]-v[-2,:-1])/(dy[-1,:-1])+\
                (v[-1,1:]-v[-2,1:])/(dy[0,1:])))\
                *0.5*(u[-1,:-1]+u[-1,1:])*Ax[-1,:-1]
        work[-1,1:]-=(2.0/3*mu*0.5*((v[-1,:-1]-v[-2,:-1])/(dy[-1,:-1])+\
                (v[-1,1:]-v[-2,1:])/(dy[0,1:])))\
                *0.5*(u[-1,:-1]+u[-1,1:])*Ax[-1,1:]
        # tau 21 * u
        work[:-1,:]+=(mu*(u[1:,:]-u[:-1,:])/(dy[:-1,:]))\
                *0.5*(u[1:,:]+u[:-1,:])*Ay[:-1,:]
        work[1:,:]-=(mu*(u[1:,:]-u[:-1,:])/(dy[:-1,:]))\
                *0.5*(u[1:,:]+u[:-1,:])*Ay[1:,:]
        work[:-1,1:-1]+=(mu*0.5*((v[:-1,2:]-v[:-1,:-2])/(dx[:-1,1:-1]+dx[:-1,:-2])+\
                (v[1:,2:]-v[1:,:-2])/(dx[1:,1:-1]+dx[1:,:-2])))\
                *0.5*(u[1:,1:-1]+u[:-1,1:-1])*Ay[:-1,1:-1]
        work[1:,1:-1]-=(mu*0.5*((v[1:,2:]-v[1:,:-2])/(dx[1:,1:-1]+dx[1:,:-2])+\
                (v[:-1,2:]-v[:-1,:-2])/(dx[:-1,1:-1]+dx[:-1,:-2])))\
                *0.5*(u[1:,1:-1]+u[:-1,1:-1])*Ay[1:,1:-1]
        work[:-1,-1]+=(mu*0.5*((v[:-1,-1]-v[:-1,-2])/(dx[:-1,-1])+\
               (v[1:,-1]-v[1:,-2])/(dx[1:,-1])))\
                *0.5*(u[1:,-1]+u[:-1,-1])*Ay[:-1,-1]
        work[1:,-1]-=(mu*0.5*((v[1:,-1]-v[1:,-2])/(dx[1:,-1])+\
               (v[:-1,-1]-v[:-1,-2])/(dx[:-1,-1])))\
                *0.5*(u[1:,-1]+u[:-1,-1])*Ay[1:,-1]
        work[:-1,0]+=(mu*0.5*((v[:-1,1]-v[:-1,0])/(dx[:-1,0])+\
                (v[1:,1]-v[1:,0])/(dx[1:,0])))\
                *0.5*(u[1:,0]+u[:-1,0])*Ay[:-1,0]
        work[1:,0]-=(mu*0.5*((v[1:,1]-v[1:,0])/(dx[1:,0])+\
                (v[:-1,1]-v[:-1,0])/(dx[:-1,1:-1]+dx[:-1,0])))\
                *0.5*(u[1:,-1]+u[:-1,-1])*Ay[1:,0]
        # tau 22 * v
        work[:-1,:]+=(4.0/3*mu*(v[1:,:]-v[:-1,:])/(dy[:-1,:]))\
                *0.5*(v[:-1,:]+v[1:,:])*Ay[:-1,:]
        work[1:,:]-=(4.0/3*mu*(v[1:,:]-v[:-1,:])/(dy[:-1,:]))\
                *0.5*(v[:-1,:]+v[1:,:])*Ay[1:,:]
        work[:-1,1:-1]+=(2.0/3*mu*0.5*((u[:-1,2:]-u[:-1,:-2])/(dx[:-1,1:-1]+dx[:-1,:-2])+\
                (u[1:,2:]-u[1:,:-2])/(dx[1:,1:-1]+dx[1:,:-2])))\
                *0.5*(v[:-1,1:-1]+v[1:,1:-1])*Ay[:-1,1:-1]
        work[1:,1:-1]-=(2.0/3*mu*0.5*((u[1:,2:]-u[1:,:-2])/(dx[1:,1:-1]+dx[1:,:-2])+\
                (u[:-1,2:]-u[:-1,:-2])/(dx[:-1,1:-1]+dx[:-1,:-2])))\
                *0.5*(v[:-1,1:-1]+v[1:,1:-1])*Ay[1:,1:-1]
        
        work[:-1,0]+=(2.0/3*mu*0.5*((u[:-1,1]-u[:-1,0])/(dx[:-1,0])+\
                (u[1:,1]-u[1:,0])/(dx[1:,0])))\
                *0.5*(v[:-1,0]+v[1:,0])*Ay[:-1,0]
        work[1:,0]-=(2.0/3*mu*0.5*((u[1:,1]-u[1:,0])/(dx[1:,0])+\
                (u[:-1,1]-u[:-1,0])/(dx[:-1,1:-1]+dx[:-1,0])))\
                *0.5*(v[:-1,0]+v[1:,0])*Ay[1:,0]
        work[:-1,-1]+=(2.0/3*mu*0.5*((u[:-1,-1]-u[:-1,-2])/(dx[:-1,-1])+\
                (u[1:,-1]-u[1:,-2])/(dx[1:,-1])))\
                *0.5*(v[:-1,-1]+v[1:,-1])*Ay[:-1,-1]
        work[1:,-1]-=(2.0/3*mu*0.5*((u[1:,-1]-u[1:,-2])/(dx[1:,-1])+\
                (u[:-1,-1]-u[:-1,-2])/(dx[:-1,-1])))\
                *0.5*(v[:-1,-1]+v[1:,-1])*Ay[1:,-1]
        # tau 12 * v
        work[:,:-1]+=(mu*(v[:,1:]-v[:,:-1])/(dx[:,:-1]))\
                *0.5*(v[:,:-1]+v[:,1:])*Ax[:,:-1]
        work[:,1:] -=(mu*(v[:,1:]-v[:,:-1])/(dx[:,:-1]))\
                *0.5*(v[:,:-1]+v[:,1:])*Ax[:,1:]
        work[1:-1,:-1]+=(mu*0.5*((u[2:,:-1]-u[:-2,:-1])/(dy[1:-1,:-1]+dy[:-2,:-1])+\
                (u[2:,1:]-u[:-2,1:])/(dy[1:-1,1:]+dy[:-2,1:])))\
                *0.5*(v[1:-1,:-1]+v[1:-1,1:])*Ax[1:-1,:-1]
        work[1:-1,1:]-=(mu*0.5*((v[2:,1:]-v[:-2,1:])/(dy[1:-1,1:]+dy[:-2,1:])+\
                (u[2:,:-1]-u[:-2,:-1])/(dy[1:-1,:-1]+dy[:-2,:-1])))\
                *0.5*(v[1:-1,:-1]+v[1:-1,1:])*Ax[1:-1,1:]
        work[0,:-1]+=(mu*0.5*((u[1,:-1]-u[0,:-1])/(dy[0,:-1])+\
                (u[1,1:]-u[0,1:])/(dy[0,1:])))\
                *0.5*(v[0,:-1]+v[0,1:])*Ax[0,:-1]
        work[0,1:]-=(mu*0.5*((u[1,:-1]-u[0,:-1])/(dy[0,:-1])+\
                (u[1,1:]-u[0,1:])/(dy[0,1:])))\
                *0.5*(v[0,:-1]+v[0,1:])*Ax[0,1:]
        work[-1,:-1]+=(mu*0.5*((u[-1,:-1]-u[-2,:-1])/(dy[-1,:-1])+\
                (u[-1,1:]-u[-2,1:])/(dy[0,1:])))\
                *0.5*(v[-1,:-1]+v[-1,1:])*Ax[-1,:-1]
        work[-1,1:]-=(mu*0.5*((u[-1,:-1]-u[-2,:-1])/(dy[-1,:-1])+\
                (u[-1,1:]-u[-2,1:])/(dy[0,1:])))\
                *0.5*(v[-1,:-1]+v[-1,1:])*Ax[-1,1:]
